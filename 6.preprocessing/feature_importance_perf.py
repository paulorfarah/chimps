from mdi import mdi
from information_gain import information_gain_cpmp, information_gain_perf
import pandas as pd
...
#     plt.axes().set_ylabel('')
    plt.savefig('results/perf/top1.pdf')
    # plt.close()
    ...
    plt.savefig('results/perf/top5.pdf')
    plt.close()
...
def plot_barh(df, filename):
    ...
    for p in graph:
        ...
        i += 1
    plt.tight_layout()
    plt.savefig('results/perf/' + filename)
...
if __name__ == "__main__":
    ...
    projects = ['commons-bcel', 'commons-csv', 'easymock', 'jgit', 'Openfire']
    ...
    main_columns = [
        ...
        "TOTAL_DECLARATIONPARTCHANGES", "TOTAL_CHANGES", "will_change", "perf_change"
    ]
    ...
    # df1.to_csv('results/perf/mdi-df1.csv')
    #
    # df5 = res[res['rank'] <= 5]
    ...
    # df5.to_csv('results/perf/mdi-top5.csv')

    df5 = res[res['rank'] <= rank]
    df5 = pd.DataFrame(df5.metric.value_counts())
    df5.columns = ['count']
    # print(df1.index)
    ...
    df5.sort_values(by=['percent'], ascending=False)
    df5 = df5.rename_axis('metrics').sort_values(by=['percent', 'metrics'], ascending=[True, False])
    plot_barh(df5, 'mdi-top' + str(rank) + '.pdf')
    df5.to_csv('results/perf/mdi-top' + str(rank) + '.csv')


    ### information gain
    res = information_gain_perf(projects, main_columns, model2)
    ...
    # df1.to_csv('results/perf/infgian-top1.csv')
    #
    # df5 = res[res['rank'] <= 5]
    # df5 = pd.DataFrame(df5.metric.value_counts())
    # df5.columns = ['count']
    ...
    # df5.to_csv('results/perf/infgain-top5.csv')
    ...
    df5.sort_values(by=['percent'], ascending=False)
    df5 = df5.rename_axis('metrics').sort_values(by=['percent', 'metrics'], ascending=[True, False])
    plot_barh(df5, 'infgain-top' + str(rank) + '.pdf')
    df5.to_csv('results/perf/infgain-top' + str(rank) + '.csv')

...
df = pd.read_csv('results/' + project_name + '-pic-all-releases-inner.csv')
df.columns = cols
...
if len(df.index) and len(df_perf.index):
    ...
    df_joined.to_csv('results2/' + project_name + '-perf-diff-all.csv', index=False)
    df_joined_cd_perf = df_joined
    df_joined_cd_perf.loc[df['will_change'] == 0, 'perf_change'] = 0
    df_joined_cd_perf.to_csv('results2/' + project_name + '-cd-perf-diff-all.csv', index=False)
print('end.')
